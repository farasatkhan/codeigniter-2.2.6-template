...
class CI_Migration {
	...
	public function __construct($config = array())
	{
		...
		log_message('Migrations class initialized', 'debug');
	...
	public function version($target_version)
	{
		...
		for ($i = $start; $i != $stop; $i += $step)
		{
			...
			if (preg_match('/^\d{3}_(\w+)$/', $name, $match))
			{
				...
				if ( ! is_callable(array(new $class, $method)))
				{
		...
		log_message('Current migration: ' . $current_version, 'debug');
		...
		log_message('Migrating from ' . $method . ' to version ' . $version, 'debug');
		...
		log_message('Finished migrating to '.$current_version, 'debug');
